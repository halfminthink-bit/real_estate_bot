...
# -*- coding: utf-8 -*-
"""
WordPress REST APIを使った自動投稿モジュール（ArticleManager版）
...
def main():
    ...
    parser = argparse.ArgumentParser(description='WordPress自動投稿（ArticleManager版）')
    parser.add_argument('--limit', type=int, help='投稿件数制限')
    parser.add_argument('--db', default='projects/setagaya_real_estate/articles.db', 
                       help='ArticleManagerデータベースパス')
    ...
    # プロジェクトディレクトリを取得（DBパスから推測）
    project_dir = db_path.parent
    
    # WordPressPublisher初期化
    try:
        publisher = WordPressPublisher(article_manager, project_dir=project_dir)
    ...
    # 投稿実行
    result = publisher.publish_all(limit=args.limit, republish=args.republish)
